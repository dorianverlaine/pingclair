...
*   📝 **Pingclairfile 配置语言**: 专为本项目设计的配置 DSL（领域特定语言）。它受到 Rust 语法启发，支持**宏 (Macros)**、**变量**和**强类型检查**，让配置复用变得前所未有的简单。
...
server "example.com" {
    ...
    route {
        ...
        match path("/api/v1/*") {
            ...
            // 转发给后端服务
            proxy "http://backend-api:3000";
        }

        // 2. 匹配静态资源（图片等）
        match path("/assets/*") || path("/images/*") {
            // 开启缓存
            headers {
...
server "app.local" {
    ...
    route {
        _ => {
            // 定义一组上游服务器
            proxy ["http://10.0.0.1:8080", "http://10.0.0.2:8080"] {
                // 负载均衡策略: RoundRobin (轮询), Random, LeastConn (最少连接)
