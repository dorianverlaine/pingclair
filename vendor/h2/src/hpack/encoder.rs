...
pub struct Encoder {
    ...
    /// Reusable scratch buffer for the encoded header block of one frame.
    ///
    /// Upstream allocates a fresh `BytesMut` (with several capacity
    /// doublings) and then a `Bytes` refcount header for every HEADERS
    /// frame. A header block lives only until it is copied into the
    /// connection write buffer, so a per-connection scratch buffer removes
    /// those per-request allocations entirely on the common no-continuation
    /// path (Pingclair performance fork).
    scratch: BytesMut,
}
...
impl Encoder {
    ...
    /// Take the connection's scratch buffer for one header-block encode.
    pub(crate) fn take_scratch(&mut self) -> BytesMut {
        std::mem::take(&mut self.scratch)
    }

    /// Return a fully-encoded header block to the connection for reuse.
