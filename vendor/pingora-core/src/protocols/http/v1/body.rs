...
impl BodyReader {
    ...
    fn prepare_buf(&mut self, buf_to_rewind: &[u8]) {
        // ⚡ Reuse the previous request's buffer instead of allocating a
        // fresh 64 KiB `BytesMut` for every message body. On a keepalive
        // connection the caller has already copied every returned slice out
        // (`read_body_bytes` uses `Bytes::copy_from_slice`), and a
        // connection with overread bytes is never reused, so the old buffer
        // is exclusively owned here (Pingclair performance fork).
        let mut body_buf = self
            .body_buf
            .take()
            .unwrap_or_else(|| BytesMut::with_capacity(self.body_buf_size));
        body_buf.clear();
        // Defensive: a path that shrank the buffer (e.g. an overread split)
        // must not trip `set_len` below.
        if body_buf.capacity() < self.body_buf_size {
            body_buf.reserve(self.body_buf_size - body_buf.capacity());
        }
    ...
    fn finish_body_buf(&mut self, end_of_body: usize, total_read: usize) {
        ...
        // ⚡ Only split off overread bytes when there are any. `split_off`
        // shrinks this handle's capacity to `end_of_body`, which would throw
        // away the 64 KiB read buffer on every exact-length response and
        // defeat the cross-request buffer reuse in `prepare_buf` (Pingclair
        // performance fork). Overread connections are never reused, so the
        // split path is cold.
        if total_read > end_of_body {
            let overread_bytes = body_buf_mut.split_off(end_of_body);
            self.body_buf_overread = Some(overread_bytes);
        }
    }
