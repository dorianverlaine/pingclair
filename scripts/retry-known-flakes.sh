...
# 🎲 Run a nextest command under the known-flaky retry policy.
#
# The websocket tunnel test is a known upstream (Pingora) flake, so the command
# is rerun only when every failure is a known flake. The allowlist carries an
# ISO admission date and expires after 30 days: a flake that survives that long
# is a bug, not a flake. The test source is never modified for this.
#
# 📌 The policy lives here and not in the workflow, because two callers need it:
# `run-ci-tests.sh` for the whole suite, and the nextest shards in
# `rust-ci-full-nextest-platform.yml`, which run one partition of an archive and
# had no policy at all — a single appearance of the known flake turned that shard
# red on 2026-09-22 while the suite job would have retried it.
...
max_attempts=3
attempt=0
log="$(mktemp "${TMPDIR:-/tmp}/nextest.XXXXXX")"
status=1
while [ "$attempt" -lt "$max_attempts" ]; do
  attempt=$((attempt + 1))
  set +e
  # 🧭 The command runs from the repository root, which is where `just test` and
  # the archive-relative paths of a shard both expect to start.
  (cd "${repository_root}" && "$@") >"$log" 2>&1
  status=$?
  set -e
  if [ "$status" -eq 0 ]; then
  ...
  if [ "$attempt" -lt "$max_attempts" ]; then
    echo "⚠️  ${failed} is/are known upstream flake(s) (attempt ${attempt}/${max_attempts}); rerunning"
  fi
done
