...
| --- | ---: | ---: | ---: |
| H1 靜態 | 46,125 | 39,478 | 18,266 |
| H1S 靜態 | 40,721 | 31,508 | 19,978 |
| H2 靜態 | 92,369 | 41,972 | 17,424 |
| H3 靜態 | 56,180 | 55,638 | 22,912 |
| H1 反向代理 | 20,856 | 22,584 | 17,117 |
| H1S 反向代理 | 20,297 | 21,944 | 16,660 |
| H2 反向代理 | 23,181 | 20,396 | 未完成 |
| H3 反向代理 | 28,078 | 22,213 | 未完成 |

Pingclair 在所有靜態列領先（H1 1.2×、H1S 1.3×、H2 2.2×、H3 打平），H2/H3
反向代理也領先（+14%／+26%）；純 H1/H1S 反向代理仍落後約 8%。Caddy 在本機
未能完成反向代理的 H2/H3 兩列（它的上游連線churn 在該並行量下會耗盡容器
的臨時埠），因此那兩格直接寫明，而不是拿不同工作量去比。筆電上的絕對
req/s 不是容量宣稱，比值才是。逐次執行的原始證據保留在本機
`benchmarks/results/20260922_mac_orbstack_2cpu/`，不在倉庫內。
