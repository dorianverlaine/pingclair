...
最新對比：Pingclair `v0.2.0-rc.3`（release 二進位）對 nginx 1.31.6 與
Caddy 2.11.4，於一台 Apple M2 筆電上以 OrbStack 測量。每個候選者都跑在
**限制 2 顆 CPU** 的容器裡、執行緒數相同、payload 同樣是 1 KiB；反向代理
的後端獨立跑在同一個 Docker network 的另一個容器；負載產生器則在原生
macOS 上執行。HTTP/1.1、HTTPS/1.1、HTTP/2 用原生 `h2load`，HTTP/3 用同一
network 內建 ngtcp2 的 `h2load`。每列為三輪交錯、每輪 50,000 請求，**全部
成功才計入**；表格中的每個數字都是這三輪的中位數。
...
Pingclair 在 HTTP/1.1、HTTPS/1.1、HTTP/2 的靜態列領先（分別是 1.2×、1.3×、
2.2×），HTTP/3 靜態則是實質打平。反向代理方面，HTTP/2 領先 14%、HTTP/3
領先 26%，而 HTTP/1.1 與 HTTPS/1.1 仍落後 nginx 約 8%。

在本機與這套 harness 的條件下，Caddy 未能完成反向代理的 HTTP/2 與 HTTP/3
兩列 —— 其上游連線 churn 在受測並行量下耗盡了容器的可用臨時埠 —— 因此那
兩格標成「未完成」，而不是拿不同工作量去比。筆電上的絕對 req/s 不是容量
宣稱；這些比值只描述「在這個受控工作量下」的相對表現，而且每一列只適合
做**同一協定內**的伺服器比較：HTTP/3 的負載產生器跑在不同環境，所以不同
協定之間的絕對吞吐不應互相比較。
...
> 校驗和。
>
> 🪦 **`v0.1.x` 已不再維護。** 不會有 `v0.1.8`：這條分支不再有修正、不再回溯，
> 也不會有安全通告，留在上面就等於留在一個沒有人會修補的版本。其中一個必須
> 升級的理由是：它的管理 API 完全沒有驗證——`v0.1.x` 的設定會解析 `api_key`，
> 卻從未讀取，所以這個欄位什麼都保護不了。如果你還在跑 `v0.1.7`，請升級。
...
簽發它需要的憑證；找到舊樹時它會留一行警告，讓這件事不是靜默的。每個信任舊根的
用戶端都必須拿到新的根——上面那幾道指令就是這一步。如果你的設定同時向公開 CA
取得憑證，重新簽發會計入該 CA 的速率限制。
...
那個寫法）會把這台伺服器自己的記錄導向指定輸出：`output file <路徑>`（不存在就
建立，權限 0600，不含顏色逃脫碼）、`output stdout`、`output stderr`、
`format json|text`，以及 `level`。讀取設定時套用一次，之後每次 reload 都重新
套用，啟動時也會有一行說明記錄去了哪裡。`RUST_LOG` 仍然優先於設定裡的
`level`——這正是它能用來把某個模組調安靜、而不必改設定檔的原因。
`🚀 Pingclair running...` 這行橫幅無論如何都留在 stdout：監督程序在那裡讀它，
把記錄送進檔案的設定不該讓看顧行程的東西失去訊號。
...
為止。下面清單裡的每一個名字都是**認得的**：寫了會得到一個指名前者的錯誤，
而不是 `Unknown directive`，用到它們的設定**啟動不了**。

📌 這個承諾只涵蓋**這些名字，不多也不少**。它不對子選項、也不對「寫在區塊
裡面」的拼法作任何保證——那並不是漏洞，而是每一種都有它自己寫在正確位置上的
說明：`tls { … }` 區塊的拒絕清單在下方；日誌文法那節說明 `log` 區塊接受哪些
形狀；同一個指示在不同層級可能一個被
接受、另一個被拒絕。寫得比這更多的句子，等於在替整個設定介面打包票，
而這個章節存在的目的正是避免那種宣稱。
...
- **DNS-01 只出貨一個 provider：Cloudflare。** 在站台層級的 `tls { … }`
  ...
  移到 DNS-01，80 埠不通的機器照樣簽得到。`*.example.com` 站台簽的就是那張
  萬用字元憑證，底下每個名稱共用同一張；apex（`example.com`）也要服務的話，
  得把它一起寫進站台名稱，因為萬用字元只涵蓋一層 label。
...
正是這個排序讓它藏得住。在閒置的十核機器上，升級測試四十次全過；在兩核容器裡四十次
失敗六次；而只要在上游送出 `101` 之前插入任何延遲——哪怕只是一次純粹的 yield——失敗
就完全消失。所以開發機會告訴你這個缺陷不存在。

沒有任何設定可以避開它。從外面看到的失敗樣子是：`101` 之後連線立刻被拆掉，兩端同時
看到 EOF，沒有錯誤訊息。
