...
/// 🗂️ Picks the first `try_files` candidate that exists under `root`.
///
/// Returns the candidate as a URI path, ready to be handed to a rewrite, or
/// `None` when nothing matched and the request should continue untouched.
///
/// The candidates are URI paths and `root` is a filesystem path, so the join
/// has to drop the candidate's leading slash: `/index.html` under `/srv` is
/// `/srv/index.html`, not `/index.html`. Getting that wrong is not a subtle
/// failure — it silently looks in the filesystem root, finds nothing, and the
/// single-page-application pattern answers 404 for every route that is not a
/// real file. That is precisely what this handler did before 2026-08-07, back
/// when only JSON could reach it.
///
/// 🛡️ Confinement is lexical and happens at configuration time: `validate_config`
/// refuses a candidate containing a `..` segment, and the request path has
/// already been normalized, so `{path}` cannot introduce one either. Nothing
/// here canonicalizes, because that would be a syscall per candidate per
/// request to re-establish something already known.
///
/// 📏 A trailing slash decides *what kind* of entry counts, and the slash that
/// decides is the one written in the configuration — not the one the request
/// happened to arrive with. The rule is read from the file matcher upstream
/// (`fileserver/matcher.go` at `ff6da121`): the expanded candidate is cleaned,
/// which drops any trailing slash, and the slash is restored only when the
/// configured pattern ended in one. Then a candidate ending in `/` matches
/// only a directory, and a candidate not ending in `/` matches only a
/// non-directory.
///
/// That distinction is the whole behaviour of the documented pattern, and it
/// is not guessable from the outside. Treating "exists" as "either kind" makes
/// `try_files {path} /index.html` match the *directory* `/assets/` for a
/// request to `/assets/`, rewrite to it, and then answer 404 from the file
/// server because the directory holds no index — where the format serves the
/// application shell. Measured against Caddy v2.11.4 on 2026-08-07: that was
/// the one request out of six where the two disagreed.
pub(crate) fn resolve_try_files(
    candidates: &[String],
    root: Option<&str>,
    request_path: &str,
) -> Option<String> {
    // 🍃 The query is not part of any filename, and on HTTP/3 the caller's
    ...
    for candidate in candidates {
        // 🍃 Only substitute when the placeholder is actually there, so the
        // common `try_files {path} /index.html` allocates once, not twice.
        let expanded = if candidate.contains("{path}") {
            Cow::Owned(candidate.replace("{path}", path))
        } else {
            Cow::Borrowed(candidate.as_str())
        };
        let wants_directory = candidate.ends_with('/');
        let mut cleaned = clean_path(&expanded);
        if wants_directory && !cleaned.ends_with('/') {
            cleaned.push('/');
        }

        let Ok(metadata) = root.join(cleaned.trim_start_matches('/')).metadata() else {
            continue;
        };
        if metadata.is_dir() == wants_directory {
            return Some(cleaned);
        }
    }
    None
...
/// 🧹 Collapses `.`, `..`, and repeated separators, the way `path.Clean` does.
///
/// The result never keeps a trailing slash, which is why the caller puts one
/// back when the configured pattern asked for a directory. `..` cannot escape
/// the root: popping past the start leaves the root, matching both the
/// upstream cleaner and this crate's request-path normalization.
...
mod tests {
    ...
    fn an_existing_file_is_returned_unchanged() {
        let root = spa_root();
        let files = candidates(&["{path}", "/index.html"]);
        assert_eq!(
            resolve_try_files(&files, root.path().to_str(), "/assets/app.js"),
    ...
    fn a_missing_file_falls_to_the_next_candidate() {
        ...
        let files = candidates(&["{path}", "/index.html"]);
        assert_eq!(
            resolve_try_files(&files, root.path().to_str(), "/deep/client/route"),
            Some("/index.html".to_string())
        );
    }

    /// 🤡 The defect this whole handler was rewritten for: candidates used to
    ...
    fn candidates_resolve_under_the_root_not_the_filesystem_root() {
        let root = spa_root();
        assert_eq!(
            resolve_try_files(&candidates(&["/index.html"]), root.path().to_str(), "/x"),
            Some("/index.html".to_string())
        );
        assert_eq!(
            resolve_try_files(&candidates(&["/index.html"]), Some("/nonexistent"), "/x"),
            None,
    ...
    fn nothing_matching_leaves_the_request_alone() {
        let root = spa_root();
        assert_eq!(
            resolve_try_files(&candidates(&["{path}"]), root.path().to_str(), "/missing"),
            None
        );
    }
    ...
    fn a_slashed_candidate_matches_only_a_directory() {
        let root = spa_root();
        let slashed = candidates(&["{path}/"]);
        for request in ["/assets", "/assets/"] {
            assert_eq!(
                resolve_try_files(&slashed, root.path().to_str(), request),
                Some("/assets/".to_string()),
                "`{{path}}/` must match the directory for {request}"
            );
        }
        assert_eq!(
            resolve_try_files(&slashed, root.path().to_str(), "/index.html"),
            None,
    ...
    fn an_unslashed_candidate_does_not_match_a_directory() {
        ...
        let files = candidates(&["{path}", "/index.html"]);
        assert_eq!(
            resolve_try_files(&files, root.path().to_str(), "/assets/"),
            Some("/index.html".to_string()),
