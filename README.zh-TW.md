...
我們在 Docker Bridge 內部網路（排除主機網路堆疊的額外開銷）中，對 Pingclair、Nginx 與 Caddy 進行了公平的壓力測試。

**測試環境**
*   硬體：MacBook Pro (M2 晶片)、Docker Desktop
*   設定：1 KB 靜態檔案、4 執行緒、100 連線、持續 15 秒
*   網路：容器對容器直連

| 伺服器 | RPS（請求／秒） | 平均延遲 | 備註 |
|--------|-----------------|----------|------|
| **Nginx (Alpine)** | **~24,902** | **4.17 ms** | ⭐️ 業界標竿，極致的 C/epoll 最佳化 |
| **Pingclair (Debian)** | **~19,899** | **5.44 ms** | 🚀 緊追在後，約達 Nginx 的 80% |
| **Caddy (Alpine)** | **~6,803** | **14.86 ms** | 🐢 易用性優先，受限於 Go GC 開銷 |

> **分析**
> Pingclair 是相對年輕的 Rust 專案，但得益於 Pingora 紮實的核心，即使在未特別最佳化的 Docker 環境下，效能仍達到成熟競品 Nginx 的 80%，並且是同樣主打易用性的 Caddy 的約 **3 倍**。
>
> *基準測試僅供參考，實際生產環境的效能取決於你的業務邏輯。*
