...
> **比表格數字更重要的但書**
> 在這個容器限流的測試環境下，反向代理吞吐量測試中 pingclair 在較高並發時反而領先 nginx 和 caddy——這個結果是真實的，但尚未在無限流的硬體上驗證，不應當作一般性結論。另外，大檔案（20MB）gzip 壓力測試揪出並促成修復了一個真實的臭蟲：靜態檔案壓縮原本每次請求都從頭壓縮、完全沒有快取，在持續並發負載下，原本 20 秒的測試跑了 16 分鐘。修好之後（加了壓縮結果快取），同一個測試現在 20.09 秒內就跑完，處理了 **21,684 個請求**（原本只有 54 個）——比 nginx 加 caddy 加起來還多，因為重複命中直接跳過壓縮。完整版裡還記錄了一個殘留的但書（並發首次未命中時「驚群」造成的暫時性記憶體峰值），請見
> [`benchmarks/README.md`](benchmarks/README.md) 的完整前後對比。
