...
|------|-----------|-------|-------|
| 靜態 1KB、純文字（c200） | 18,795 req/s | **55,236 req/s** | 17,413 req/s |
| 靜態 1KB、gzip（c200） | 27,459 req/s | **44,268 req/s** | 15,412 req/s |
| 反向代理（c200） | 17,716 req/s | **20,778 req/s** | 9,168 req/s |
| 大檔案 20MB、gzip（c20） | **703 req/s、0 逾時** | 9.1 req/s、110 逾時 | 10.1 req/s、65 逾時 |

**如何解讀**

- 在純文字小檔案靜態服務上，Nginx 領先約 2.6-2.9 倍（sendfile 加上成熟的熱路徑）——
  這個差距是真實的，也是 pingclair 最有改進空間的地方。同場景下 Pingclair 略勝 Caddy。
- 反向代理與 nginx 基本打平（約 84-99%），約為 Caddy 的 1.8-2 倍，三者均零錯誤。
