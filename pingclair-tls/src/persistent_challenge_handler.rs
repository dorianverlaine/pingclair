...
impl PersistentChallengeHandler {
    ...
    pub async fn cleanup_expired(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let removed_count = {
            ...
            tokens.retain(|_, entry| current_time - entry.created_at < TOKEN_TTL_SECS);
            before - tokens.len()
        };
...
impl crate::acme::ChallengeHandler for PersistentChallengeHandler {
    async fn deploy(
        &self,
        challenge: &crate::acme::ChallengeResponse,
    ) -> Result<(), crate::acme::AcmeError> {
        self.store_token(challenge.token.clone(), challenge.key_authorization.clone())
            .await
            .map_err(|error| {
...
mod tests {
    ...
    async fn test_persistent_storage() {
        ...
        let handler = PersistentChallengeHandler::new(storage_path.clone())
            .await
            .unwrap();
        ...
        let handler2 = PersistentChallengeHandler::new(storage_path.clone())
            .await
            .unwrap();
        ...
        let handler3 = PersistentChallengeHandler::new(storage_path.clone())
            .await
            .unwrap();
